...
每次运行都会记录在系统的 Verdikt 数据目录中。Windows 默认是 `%LOCALAPPDATA%\\Verdikt`，macOS 默认是 `~/Library/Application Support/Verdikt`，Linux 默认是 `$XDG_STATE_HOME/verdikt` 或 `~/.local/state/verdikt`。如果旧版本在启动项目的 `.verdikt` 中留下过记录，首次启动时会把它们复制到新目录，旧目录会保留不动。常见内容包括：
...
工作台会把排队、运行、等待确认和可恢复状态保存在同一个 Verdikt 数据目录中。正常关闭工作台时，正在执行的任务会先保存现场；再次启动后，排队任务会自动继续，可恢复任务也会重新进入队列。主动点击“停止运行”仍然表示真正取消，并会按取消规则清理现场。需要改位置时可设置 `VERDIKT_STATE_DIR`。
