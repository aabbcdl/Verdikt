...
export async function runSupervisorLoop(
  ...
  // If resuming, delegate to resumeSupervisorLoop
  ...
  // Concurrency lock
  const locked = acquireLock(stateDir, task.repoPath, runId);
  if (!locked) {
    const holder = await import("../workspace/lock.js").then((m) =>
      m.checkLock(stateDir, task.repoPath),
    );
    throw new Error(
  ...
  return executeLoop(task, runId, runDir, stateDir, options);
}
