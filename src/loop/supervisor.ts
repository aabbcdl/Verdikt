...
import { createRunId, initRun, recordIteration, writeSummary, saveRunState, loadRunState, clearRunState } from "../trace/recorder.js";
...
export async function runSupervisorLoop(
  ...
  let workDir = task.repoPath; // Default: work in the original repo
  ...
  if (useIntegrity) {
    ...
    log(`  ▸ Baseline: ${baseline.fileHashes.size} test files, ${baseline.assertionCounts.size} assertion groups`);
  }

  const iterations: IterationRecord[] = [];
  let instruction = resumeState?.instruction ?? task.goal;
  let totalCost = resumeState?.totalCostUsd ?? 0;
  let integrityViolations: Array<{ iteration: number; violations: string[] }> = [];
  ...
  const startTime = resumeState?.totalDurationMs ? Date.now() - resumeState.totalDurationMs : Date.now();
  ...
  try {
    for (let i = startIteration; i < task.maxIterations; i++) {
      ...
      const execResult = await runExecutor({ ...task, repoPath: workDir }, instruction, useStreaming ? {
        onChunk: (text) => {
      ...
      log(`  ▸ Executor done (${execResult.durationMs}ms${execResult.costUsd ? `, $${execResult.costUsd.toFixed(4)}` : ""})`);
      ...
      if (task.maxBudgetUsd) {
        ...
        if (pct >= 1.0) {
          ...
          warn(`  💰 Budget warning: $${totalCost.toFixed(2)} / $${task.maxBudgetUsd.toFixed(2)} (${(pct * 100).toFixed(0)}%)`);
        }
      }
      ...
      if (useWorktree && worktreeInfo) {
        const diff = await captureIterationDiff(workDir, worktreeInfo.evidenceDir, i, preExecutorCommit);
        ...
        log(`  ▸ Patch: ${patchPath} (${changedFiles.length} files, +${iterLinesAdded}/-${iterLinesDeleted})`);
      } else {
      ...
      let semanticFindings: import("../workspace/semantic-scanner.js").SemanticRiskResult | null = null;
      if (task.semantic?.maxRisk && useWorktree && worktreeInfo) {
        try {
          ...
          if (actualRisk > maxAllowed && judge.passed) {
            ...
            log(`  ⚠ SEMANTIC GATE FAILED: risk=${semanticFindings.level}, max=${task.semantic.maxRisk}`);
      ...
      const effectiveJudge = semanticGateFailed
        ? { ...judge, passed: false, checks: [...judge.checks, { name: "semantic-risk", passed: false, output: `Semantic risk ${semanticFindings?.level} exceeds max ${task.semantic?.maxRisk}`, exitCode: 1, durationMs: 0 }] }
        : judge;
      ...
      const verifierResult = await runVerifier({ ...task, repoPath: workDir }, effectiveJudge, execResult.text);
      const verdict = verifierResult.verdict;
      if (verifierResult.costUsd) totalCost += verifierResult.costUsd;
      log(`  ▸ Verifier: done=${verdict.done}, problems=${verdict.problems.length}${verifierResult.costUsd ? `, $${verifierResult.costUsd.toFixed(4)}` : ""}`);
      ...
      if (decision.stop) {
        ...
        let applyStatus: "applied" | "discarded" | "pending" = "pending";
        ...
        const result: RunResult = {
          ...
          runId,
          taskId: task.id,
          workspace: useWorktree && worktreeInfo ? {
          ...
          integritySummary: integrityViolations.length > 0
            ? { status: "violations", criticalCount: integrityViolations.reduce((s, v) => s + v.violations.length, 0), warningCount: 0, issues: integrityViolations.flatMap((v) => v.violations.map((d) => ({ rule: "violation", detail: d }))) }
      ...
      log(`  ▸ Next round instruction: ${instruction.slice(0, 200)}${instruction.length > 200 ? "..." : ""}\n`);
    }
